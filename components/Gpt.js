...
export default function Gpt() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "system",
      content: "Pretend you are a Doctor. Answer with medical content",
    },
      ...
      <main className="mx-auto h-screen max-w-full sm:max-w-3xl ">
        ...
        {messages.length === 1 && (
          <div className="mx-10 mt-20 flex justify-center">
            <div>
              <p className="mb-2 font-bold">Talk to the AI Doctor</p>
              <p className="mb-32">
        ...
        <div
          ref={backgroundRef}
          className="fixed bottom-0 z-10 w-full max-w-full bg-transparent
                     sm:max-w-3xl"
        ></div>

        <div
          className="fixed bottom-5 z-20 w-full max-w-full 
                     sm:max-w-3xl flex justify-center"
        >
          <form className=" flex items " onSubmit={onSubmit}>
            <textarea
              ref={textareaRef}
              className="mr-2  resize-none rounded-md border- 
                       border-gray-400 p-2 focus:border-blue-600 
                         focus:outline-none"
              value={newMessageText}
              onChange={onChange}
              onKeyDown={onKeyDown}
              placeholder="Why am i feeling tired?"
            />
